...
> 记录：2026-09-07。来源：对照 [实施清单](./midroute-implementation-checklist.md) 全部 MR 验收框做的全量代码核查（实跑 `./build/test.sh`、`./build/build.sh` + 四路并行代码审查）。
...
|---|---|---|---|---|
| ISS-01 | P0 | MR-002 | 管理页 404 未修复：StaticDir 是死配置，httpserver 无任何静态托管 | 未修复 |
| ISS-02 | P0 | MR-012 | router `streamed` 标志从未置 true，首块输出后防重放防护实际失效 | 未修复 |
| ISS-03 | P0 | MR-015 | 用量落库复用请求 context 且忽略错误：客户端取消即丢记录；流式中断被记为成功 | 未修复 |
| ISS-04 | P0 | MR-028 | 备份脚本不覆盖现行主库 `midroute.db`，且显式打包明文凭据 `local.keys` | 未修复 |
| ISS-05 | P0 | MR-001 | `.gitignore` 缺 `node_modules` 与 `apps/web/dist`，Git 尚无首次提交，`git add .` 会入库依赖 | 未修复 |
| ISS-06 | P0 | MR-030 | secret-scan 命中时打印整行内容（含疑似密钥本身），不满足"只报告脱敏位置" | 未修复 |
| ISS-07 | P0 | MR-002 | 无会话机制、无 Host/Origin 校验、无 CSRF 防护：localOnly 下浏览器跨站可直写管理 API | 未修复 |
| ISS-08 | P1 | MR-001 | 冒烟脚本固定端口 18100、固定数据路径 `/tmp/midroute-smoke` 且启动即 `rm -rf`，可误伤已有实例 | 未修复 |
...
| ISS-15 | P1 | MR-019 | `price_versions` 表有 schema 无任何读写代码；金额用 float64 累加；对账不校验币种/范围 | 未修复 |
| ISS-16 | P1 | MR-023 | 权重路由未实现：`Weight` 字段存在但从不参与选择，仓储写死 `weight=1, priority=0` | 未修复 |
| ISS-17 | P2 | MR-012/014 | Anthropic 流式翻译只处理 `text_delta`，工具调用块（`input_json_delta` 等）被静默丢弃 | 未修复 |
...
### ISS-17 Anthropic 流式丢工具块（P2 · MR-012/014）
...
1. **先做 ISS-05**（.gitignore + 首次提交）：成本最低，且拖越久依赖越容易入库。
2. **随后 ISS-01 + ISS-07**（管理页托管 + 会话/CSRF）：MR-002 是 D0 退出条件，也是后续所有页面任务的前置。
3. **ISS-02 + ISS-03 + ISS-09/10**（转发可靠性一组）：直接影响计费正确性与首个 Coding 闭环（D2）。
4. **ISS-04 + ISS-06 + ISS-08**（脚本安全一组）：备份/扫描/冒烟都是交付门禁，改动小。
5. ISS-11–ISS-19 随对应 MR 任务开发时一并处理。
